function navBar(strData){
	var data;
	var beforeData;
	var menulist= getmenu().data.data;
	console.log(menulist)
	var administerType=localStorage.getItem("administerType");
	...
	if(administerType!='0'){
		for(var i=0;i<strData.length;i++){
			for (var m = 0; m < menulist.length; m++) {
				if(menulist[m].menuId==strData[i].power){
					data.push(strData[i])
				}
			}
		}		
...
function  getmenu(){
	...
	$.ajax({
		...
		async:false,
		data: {
			adminId: localStorage.getItem("administerId"),
		}, 
